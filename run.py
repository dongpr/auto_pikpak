...
# ============邮箱接口函数，已对接4个临时邮箱接口============
# 临时邮箱2不同后缀的数组
domains2 = ["mailto.plus", "fexpost.com", "fexbox.org", "rover.info", "chitthi.in", "fextemp.com",
...
# 获取邮箱1的验证码内容
def get_verification_code1(mail, max_retries=10, delay=1):
    retries = 0
...
# 获取邮箱2接收到的邮件ID
def get_email_first_id(email, max_retries=15, delay=1):
    ...
    while retries < max_retries:
        ...
        if data.get("mail_list"):
            # 如果mail_list不为空，获取第一个邮件的mail_id
            mail_id = data["mail_list"][0]["mail_id"]
            return mail_id
        else:
            # 如果mail_list为空，等待一段时间然后重试
            time.sleep(delay)
            retries += 1
        # 如果重试到达最大次数，返回None或者抛出异常
    print("获取2号邮箱邮件内容失败，未收到邮件...")
    return None  # 或者可以选择抛出一个异常


# 获取邮箱2的邮件内容
def get_verification_code2(first_id, email, max_retries=10, delay=1):
...
# 获取email的邮件ID
def get_email_id(email, auth_token, domain_suffix, max_retries=10, delay=1):
    api_url = f"https://mail.{domain_suffix}/api/api/v1/mailbox/{email}"
    headers = {
        "Authorization": auth_token
    }
    ...
    while retries < max_retries:
        ...
        if data:
            ...
            time.sleep(delay)
            retries += 1
    print("获取3/4号邮箱邮件内容失败，未收到邮件...")
    return None


# 获取临时邮箱3&4的验证码
def get_verification_code3(email_id, email, auth_token, domain_suffix, max_retries=10, delay=1):
...
# 获取验证码
def get_verification_code5(token, domain_suffix, max_retries=10, delay=1):
...
def start():
    global mail_token, mail_url, verification_code
    invite_code = "64639011"
    ranemcode = random.randint(1, 6)
    email_code = str(ranemcode)
